...
const toWorld = (x: number, y: number) => new THREE.Vector3((x - 100) * S, (80 - y) * S, 0);
const RADIUS = 7 * S;
...
function useMaterials() {
  return useMemo(
    () => ({
      teal: new THREE.MeshPhysicalMaterial({ color: "#19b3bf", roughness: 0.22, metalness: 0.15, clearcoat: 1, clearcoatRoughness: 0.08 }),
      orange: new THREE.MeshPhysicalMaterial({ color: "#ff5a1f", roughness: 0.2, metalness: 0.1, clearcoat: 1, clearcoatRoughness: 0.06 }),
      paper: new THREE.MeshPhysicalMaterial({ color: "#eeece6", roughness: 0.35, clearcoat: 0.6 }),
      ring: new THREE.MeshBasicMaterial({ color: "#eeece6", transparent: true, opacity: 0.22 }),
      ringFaint: new THREE.MeshBasicMaterial({ color: "#eeece6", transparent: true, opacity: 0.08 }),
      node: new THREE.MeshStandardMaterial({ color: "#ff5a1f", emissive: "#ff5a1f", emissiveIntensity: 0.6 }),
    }),
    [],
  );
}
...
function Mark({ progress, reduced }: { progress: Progress; reduced: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    const targetY = (reduced ? 0 : Math.sin(t * 0.45) * 0.35) + state.pointer.x * 0.35 + p * 1.2;
    const targetX = -state.pointer.y * 0.2 + (reduced ? 0 : Math.sin(t * 0.3) * 0.06);
    root.current.rotation.y = THREE.MathUtils.lerp(root.current.rotation.y, targetY, k * 0.08);
    root.current.rotation.x = THREE.MathUtils.lerp(root.current.rotation.x, targetX, k * 0.08);
    root.current.position.y = reduced ? 0 : Math.sin(t * 0.8) * 0.06;

    // Scroll pulls the mark apart at the seams
    if (left.current) left.current.position.x = THREE.MathUtils.lerp(left.current.position.x, -p * 0.9, k * 0.15);
    if (right.current) right.current.position.x = THREE.MathUtils.lerp(right.current.position.x, p * 0.9, k * 0.15);
    if (slash.current) slash.current.rotation.z = THREE.MathUtils.lerp(slash.current.rotation.z, -p * Math.PI * 0.5, k * 0.15);
  });
...
export default function GeekMark3D({ progress, className = "" }: { progress: Progress; className?: string }) {
  ...
  return (
    <div ref={wrap} className={className} aria-hidden>
      ...
      >
        ...
        <directionalLight position={[-5, -2, -4]} intensity={0.5} color="#ff5a1f" />
        {/* Procedural studio reflections: no HDR download */}
        <Environment resolution={256}>
          <Lightformer form="rect" intensity={3} position={[0, 4, 5]} scale={[10, 3, 1]} />
          <Lightformer form="rect" intensity={1.5} color="#ff5a1f" position={[-6, 0, 2]} rotation-y={Math.PI / 2} scale={[6, 4, 1]} />
          <Lightformer form="rect" intensity={1.2} color="#19b3bf" position={[6, -1, 2]} rotation-y={-Math.PI / 2} scale={[6, 4, 1]} />
